...
var customSortFerp = function(arr, direction) {
...
var customSortNumber = function(arr, direction) {
  ...
  return customSort(newArray, direction);
}

var customSortNumberFerp = function(arr, direction) {
  var newArray = arr.slice(0);
  newArray.map(function(val) {
    return parseFloat(val);
  })

  return customSortFerp(newArray, direction);
}
...
var customSortColorBoolean = function(arr, direction) {
...
var customSortByType = function(type, arr, direction, enumeration) {
  switch (type) {
    ...
    case 'number':
      return customSortNumber(arr, direction);
    case 'ferpNumber':
      return customSortNumberFerp(arr, direction);
    case 'ferpTime':
      return customSortNumberFerp(arr, direction);
    case 'boolean':
    ...
    case 'colorBoolean':
      return customSortColorBoolean(arr, direction);
    default:
...
var normalizeArray = function(array, type, placeholder) {
  ...
  switch (type) {
    ...
    case 'ferpTime':
      return array.map(function(val) {
        var hrs = parseInt(val.split(":")[0]);
        var mins = parseInt(val.split(":")[1]);
        var secs = parseInt(val.split(":")[2]);
...
module.exports = {
  ...
  customSortFerp: customSortFerp,
  customSortNumber: customSortNumber, 
  customSortNumberFerp: customSortNumberFerp, 
  ...
  customSortColorBoolean: customSortColorBoolean,
